...
app.post(`/admin/${ADMIN_URL_PATH}/collections`, requireAdmin, async (req, res) => {
  ...
  app.post(`/admin/${ADMIN_URL_PATH}/ai/generate-content`, requireAdmin, async (req, res) => {
    try {
      const { title, location } = req.body;
      if (!title || !location) {
        return res.status(400).json({ error: "タイトルと場所は必須です" });
      }

      const prompt = `展示会のタイトル「${title}」と開催場所「${location}」から、以下の情報を生成してください：
1. サブタイトル：展示会の特徴を簡潔に表現する一文（30文字以内）
2. 説明文：展示会の詳細な説明（200文字程度）
...
形式は以下のJSONで返してください：
{
  ...
  "description": "生成された説明文"
      ...
      if (!response) throw new Error('AIによる生成に失敗しました');
  ...
  app.post(`/admin/${ADMIN_URL_PATH}/exhibitions`, requireAdmin, async (req, res) => {
    try {
      const exhibitionData = {
        ...
        subImageUrls: Array.isArray(req.body.subImageUrls) ? req.body.subImageUrls : [],
