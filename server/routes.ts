...
app.post(`/admin/${ADMIN_URL_PATH}/collections`, requireAdmin, async (req, res) => {
  ...
  app.get("/api/voices", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "ボイスの取得に失敗しました" });
    }
  });
  ...
  app.post(`/admin/${ADMIN_URL_PATH}/voices`, requireAdmin, upload.single('image'), async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "ボイスの作成に失敗しました" });
    }
  });

  app.put(`/admin/${ADMIN_URL_PATH}/voices/:id`, requireAdmin, upload.single('image'), async (req, res) => {
    try {
      ...
      if (req.file) {
        updateData.imageUrl = `/artworks/${req.file.filename}`;
      }
      ...
      res.status(500).json({ error: "ボイスの更新に失敗しました" });
    }
  });
  ...
  app.delete(`/admin/${ADMIN_URL_PATH}/voices/:id`, requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "ボイスの削除に失敗しました" });
    }
  });
