  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        // CMYK gamut: slightly desaturate (印刷はRGBより彩度が落ちる)
        const sat = 0.80;
        let r2 = lum + (r - lum) * sat;
        let g2 = lum + (g - lum) * sat;
        let b2 = lum + (b - lum) * sat;
        // Dot gain: shadows & midtones darken (インク滲み)
        const dg = (v: number) => v < 0.5 ? v * (1 - (0.5 - v) * 0.40) : v;
        // Overall darkening (紙面はモニターより暗い)
        const dk = 0.90;
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        {isReady && (
          <div className="mt-16 flex justify-center">
            <button
              onClick={() => { setCachedTransparentPng(null); setOrderOpen(true); }}
